...
- **Update dependencies** — every Monday 07:00. Check for outdated npm dependencies in both npm projects — `addon/` and `site/` — update them, run each one's verification gate, and open a PR if everything passes. Like the architecture review it keeps **one** pull request, on a long-lived `chore/update-dependencies` branch rebased onto `main` each run — two open PRs both rewriting `package-lock.json` conflict with each other by construction. The procedure lives in the [update-dependencies skill](../.claude/skills/update-dependencies/SKILL.md); the schedule that fires it is registered per-machine (see below).
